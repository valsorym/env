...
Parser supports the following field types: `int`, `int8`, `int16`, `int32`, `int64`, `uin`, `uint8`, `uin16`, `uint32`, `uin64`, `float32`, `float64`, `string`, `bool` and `slice` <ins>from thous types</ins> *(i.e. `[]int`, `[]int8`, ..., `[]bool`)*. For other filed's types will be returned an error - **`env.TypeError`**.
